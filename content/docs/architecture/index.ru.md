...
| BIP39 | Мнемонические фразы восстановления (с кошелька 0.20.0) |
| BIP44 | Иерархия нескольких аккаунтов — зарегистрированный тип монеты **1524** |

### Зарегистрированный тип монеты (SLIP-44)

За блокчейном TALER постоянно закреплён тип монеты BIP-44:

| | |
|---|---|
| Тип монеты | **1524** |
| Усиленный (hardened) компонент пути | `0x800005F4` |
| Путь деривации | `m/44'/1524'/<аккаунт>'/<цепочка>/<индекс>` |
| Зарегистрирован | 18 декабря 2017 года |
| Реестр | [SLIP-0044 — Registered coin types for BIP-0044](https://github.com/satoshilabs/slips/blob/master/slip-0044.md) |

SLIP-0044 — это публичный реестр номеров типов монет, к которому отсылает BIP-44; его ведёт
SatoshiLabs. Запись в нём не позволяет двум проектам выводить ключи по одному и тому же пути,
и именно этот номер используют кошельки и инструменты восстановления, чтобы найти аккаунты
монеты.

Запись TALER была добавлена через три месяца после генезис-блока 13 сентября 2017 года.
Поэтому любой кошелёк TALER, следующий BIP-44 — настольный или мобильный — использует
`m/44'/1524'/…` и восстанавливает одни и те же адреса из одной и той же фразы восстановления
в любой реализации.

Обратите внимание: у GNU Taler, независимой платёжной системы, записи в SLIP-0044 нет;
тип монеты 1524 принадлежит этому проекту.
...
| Тип монеты BIP44 | 1524 (`0x800005F4`) |
